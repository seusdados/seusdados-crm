...
// Dados do questionário LGPD predefinido
const lgpdQuestionnaireData = true
...
async function migrateLGPDQuestionnaire() {
  try {
    ...
    // Autenticar como usuário admin para ter permissões necessárias
    // Vamos pedir ao usuário que forneça a senha do admin
    const ADMIN_EMAIL = 'joao@seusdados.com.br' // Usuário admin encontrado no banco de dados
    // Solicite a senha do administrador ao executar este script
    const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'admin123' // Substitua pela senha real ou use variável de ambiente
    ...
    // Obter o user_id e organization_id do usuário autenticado
    ...
    console.log(`Usuário ID: ${userId}, Organização ID: ${organizationId}`)
    
    // Verificar se os dados do questionário estão disponíveis
    if (!lgpdQuestionnaireData) {
    ...
    const { data: questionnaire, error: questionnaireError } = await supabase
      ...
      .insert({
        ...
        category: 'lgpd',
        user_id: userId,  // Adicionar o user_id do admin
        organization_id: organizationId,  // Adicionar o organization_id do admin
        is_template: true,  // Definir como template para ser reutilizável
        settings_json: {
          theme: 'default',
