...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'documentos-empresas',
        name: 'documentos-empresas',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/jpeg", "image/png", "image/webp"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for documentos-empresas" ON storage.objects FOR SELECT USING (bucket_id = 'documentos-empresas');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for documentos-empresas" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'documentos-empresas');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for documentos-empresas" ON storage.objects FOR UPDATE USING (bucket_id = 'documentos-empresas');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for documentos-empresas" ON storage.objects FOR DELETE USING (bucket_id = 'documentos-empresas');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'documentos-empresas',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/jpeg", "image/png", "image/webp"],
