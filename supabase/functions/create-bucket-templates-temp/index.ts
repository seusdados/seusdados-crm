...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/html", "application/json"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for templates" ON storage.objects FOR SELECT USING (bucket_id = 'templates');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for templates" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'templates');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for templates" ON storage.objects FOR UPDATE USING (bucket_id = 'templates');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for templates" ON storage.objects FOR DELETE USING (bucket_id = 'templates');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'templates',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/html", "application/json"],
