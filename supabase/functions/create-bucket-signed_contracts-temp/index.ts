...
Deno.serve(async (req) => {
    ...
    bucketConfig.file_size_limit = 15728640;
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for signed_contracts" ON storage.objects FOR SELECT USING (bucket_id = 'signed_contracts');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for signed_contracts" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'signed_contracts');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for signed_contracts" ON storage.objects FOR UPDATE USING (bucket_id = 'signed_contracts');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for signed_contracts" ON storage.objects FOR DELETE USING (bucket_id = 'signed_contracts');`
    ];
